...
import { swapCaseStudyPositions, neighborSlug } from "@/lib/db";
...
export async function POST(req: Request) {
  ...
  const neighbour = await neighborSlug("case_studies", slug, dir);
  if (!neighbour) return NextResponse.json({ ok: true, moved: false });
  await swapCaseStudyPositions(slug, neighbour);
